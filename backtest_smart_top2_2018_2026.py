#!/usr/bin/env python3
"""
破底翻 SMART+Top 2 Backtest — HSI Blue Chips (Apr 2018 – Mar 2026)
===================================================================
Smart filters applied:
1. Confidence ≥ 0.7
2. R:R ≥ 3.0 (Cai Sen's 3:1 rule)
3. Volume confirmation (recent > avg * 1.2)
4. MA20 uptrend
5. Skip stock if traded in last 2 months
6. Market regime: skip if HSI below 60-day MA (bear filter)
7. Liquidity: min avg daily volume 500K shares
...
# ============================================================
# Detector (from proven smart backtest)
# ============================================================
def detect_po_di_fan(ticker, name, df, scan_end_date):
    ...
    for lookback in [60, 90, 120]:
        ...
        for end_idx in range(lookback + 30, len(close)):
            ...
            # 1. Find bottom (min close in lookback)
            ...
            # Entry / Stop / Target (next-day Open)
            ...
            # Confidence scoring (from proven smart backtest)
            confidence = 0.5
            if vol_confirm:
                confidence += 0.2
            if rr >= 3:
...
# SMART Filter
# ============================================================
def apply_smart_filters(signals: List[Signal], recent_tickers: set, stock_data: dict, month_end: str) -> List[Signal]:
    filtered = []
    for s in signals:
        reasons = []
...
def run_backtest():
    print("=" * 70)
    print("破底翻 SMART+Top 2 Backtest — HSI Blue Chips")
    print(f"Period: {BACKTEST_START} to {BACKTEST_END}")
    print(f"Filters: conf>={MIN_CONFIDENCE}, R:R>={MIN_RR}, vol={REQUIRE_VOLUME}, MA↑={REQUIRE_MA_UPTREND}")
    print(f"         skip_recent={SKIP_RECENT_MONTHS}m, min_vol={MIN_AVG_VOLUME/1e6:.0f}M")
    print("=" * 70)
    ...
    for month_start in months:
        ...
        if not raw_signals:
            ...
            print(f"  {month_label}: 0 signals")
            continue

        # Apply SMART filters
        filtered = apply_smart_filters(raw_signals, recent_tickers, stock_data, month_end)
        ...
        if not picks:
            ...
            print(f"  {month_label}: {len(raw_signals)} raw → 0 passed filters")
            continue
        ...
        monthly_results.append({
            ...
            'filtered': len(filtered), 'trades': month_trades, 'picks': picks
        })

        pnl_str = f"{sum(t.pnl_pct for t in month_trades):+.2f}%" if month_trades else "n/a"
        print(f"  {month_label}: {len(raw_signals)} raw → {len(filtered)} passed → {len(month_trades)} trades, PnL={pnl_str}")
        ...
        if len(monthly_results) > SKIP_RECENT_MONTHS:
            old = monthly_results[-(SKIP_RECENT_MONTHS + 1)]
...
def compute_stats(all_trades, monthly_results):
    ...
    for mr in monthly_results:
        ...
        if trades:
            monthly_pnls.append(sum(t.pnl_pct for t in trades))
        else:
            monthly_pnls.append(0)

    sharpe = 0
    if monthly_pnls and np.std(monthly_pnls) > 0:
...
def print_stats(stats):
    print("\n" + "=" * 70)
    print("BACKTEST SUMMARY — 破底翻 SMART+Top 2")
    print("=" * 70)
...
<title>破底翻 SMART+Top 2 Backtest — HSI Blue Chips (Apr 2018 – Mar 2026)</title>
<style>
...
.vs{{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px}}
@media(max-width:768px){{.grid{{grid-template-columns:repeat(2,1fr)}}.card .value{{font-size:18px}}.vs{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="container">
<h1>🧠 破底翻 SMART+Top 2 Backtest — HSI Blue Chips</h1>
<p class="subtitle"><span>📅 Apr 2018 → Mar 2026</span><span>|</span><span>🔬 SMART Filters + Top 2</span><span>|</span><span>🎯 {stats['total_trades']} trades</span></p>
""")
...
<div class="rule">✅ <strong>Confidence ≥ 0.7</strong> — Only high-conviction signals</div>
<div class="rule">✅ <strong>R:R ≥ 3.0</strong> — Cai Sen's 3:1 risk-reward minimum</div>
<div class="rule">✅ <strong>Volume Confirmation</strong> — Recovery must show volume surge (>1.2x avg)</div>
<div class="rule">✅ <strong>MA20 Uptrend</strong> — Short-term moving average trending up</div>
<div class="rule">✅ <strong>No Repeat</strong> — Skip stock if traded in last 2 months</div>
<div class="rule">✅ <strong>Liquidity</strong> — Min avg daily volume 0.5M shares</div>
<div class="rule">📌 <strong>Entry:</strong> Next-day Open | <strong>Stop:</strong> 3% below bottom | <strong>Target:</strong> Neckline + distance</div>
</div>
""")
...
<div class="card"><div class="label">Sharpe Ratio</div><div class="value {'g' if stats['sharpe']>0 else 'r'}">{stats['sharpe']:.2f}</div></div>
<div class="card"><div class="label">Profit Factor</div><div class="value {'g' if stats['profit_factor']>1 else 'r'}">{stats['profit_factor']:.2f}</div></div>
</div>
""")
...
<table><thead><tr><th>#</th><th>Ticker</th><th>Name</th><th>Signal Date</th><th>Entry Date</th><th>Entry $</th><th>Exit Date</th><th>Exit $</th><th>Stop Loss</th><th>Target</th><th>Exit Reason</th><th>Days</th><th>PnL %</th><th>Conf</th><th>R:R</th></tr></thead><tbody>""")
    ...
    for mr in monthly_results:
        ...
        html.append(f"""<div class="ms"><div class="mh" onclick="this.nextElementSibling.classList.toggle('hidden')"><h3>{month}</h3><div class="sum"><span class="tag tn">Raw: {raw}</span><span class="tag tn">Passed: {filt}</span><span class="tag {tc}">PnL: {mp:+.2f}%</span><span class="tag {'tw2' if w==len(trades) and trades else 'tl' if w==0 and trades else 'tn'}">W/L: {w}/{len(trades)}</span></div></div><div class="mb hidden">""")
        if trades:
            ...
            html.append(f'<p style="padding:14px;color:#8b949e">{raw} signals found but all filtered out by SMART criteria.</p>')
        else:
            html.append('<p style="padding:14px;color:#8b949e">No 破底翻 signals this month.</p>')
        html.append('</div></div>')
...
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')} | Strategy: 破底翻 SMART+Top 2 | Universe: HSI Blue Chips (75 stocks) | Period: Apr 2018 – Mar 2026
</div></div>
<script>
...
if __name__ == '__main__':
    ...
    out_file = 'backtest_smart_top2_2018_2026.html'
    ...
    json_out = 'backtest_smart_top2_2018_2026.json'
    with open(json_out, 'w', encoding='utf-8') as f:
        json.dump({
            'generated': datetime.now().isoformat(),
            'strategy': '破底翻 SMART+Top 2',
